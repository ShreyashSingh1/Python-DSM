"""Static method can be used with an object of class, but it is not advisable
 To achieve memory optimization we use static method """
class PwSkills:
...
class PwSkills2:
    ...
    @classmethod
    def class_name(cls):
        """Class methods in Python are not replicated for each object
    ...
    def mentor(self, mentor_list):
        print(mentor_list)
        """Calling static method inside insight function"""
...
"""
Difference between static and class method Static methods and class methods are 
both types of methods that belong to a class rather than to an instance of the
class. However, they have some key differences in terms of behavior and usage:

->Accessing class attributes: 
Static methods cannot access or modify any instance-specific data or attributes of the 
...
->Bound to the class: 
Static methods are bound to the class itself and do not have access to the instance or the ability
...
->Method invocation: 
Static methods are called directly on the class itself, without the need to create an instance. 
